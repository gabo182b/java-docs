...
export class DocumentationSearch {
  ...
  findClass(className: string): JavaDocumentation | null {
    ...
    return (
      this.docs.find((doc) => doc.className.toLowerCase() === lowerName) || null
    );
  }
...
export async function initializeSearch(
  ...
  try {
    if (typeof window === "undefined") {
      ...
      const filePath =
        docsPath || path.join(process.cwd(), "data", "java-docs.json");

      try {
        await fs.access(filePath);
      } catch {
        console.log(
          "Documentation file not found, the chat will work without documentation context",
        );
        return search;
      }
      ...
      if (!data || data.trim().length === 0) {
        console.log(
          "Documentation file is empty, the chat will work without documentation context",
        );
        return search;
      }
